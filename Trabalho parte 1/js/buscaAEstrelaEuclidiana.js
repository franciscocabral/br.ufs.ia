...
function buscaAEstrelaEuclidiana(mapa, inicio, fim){
    echo("Executando A* Euclidiana em {"+inicio.x+", "+inicio.y+"}, procurando por {"+fim.x+", "+fim.y+"}");
    var fila = [];
    fila.push(inicio);
    ...
    var alg = function(){
        ...
        for(var k = 0; k < testPoses.length; k++) {
            ...
            if(canWalk(testPos) && !foiVisitado(testPos)) {
                var menorEuclidiana = calcDistancia(menor, fim);
                if(menor.x == actualPos.x && menor.y == actualPos.y){
                    menorEuclidiana *= 2;
                }
                if (calcDistancia(testPos, fim) < menorEuclidiana) {
                    menor = testPos;
                    possible = [];
                    possible[0] = testPos;
                } else if (calcDistancia(testPos, fim) == menorEuclidiana) {
